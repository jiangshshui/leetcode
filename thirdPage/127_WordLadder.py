...
#
#     def canSplit(self,beginWord,endWord,wordList,visited):
#         if beginWord==endWord:
#             self.res.append(self.cnt)
#             return
#         if self.diffOneCh(beginWord,endWord):
#             self.cnt+=1
#             self.res.append(self.cnt)
#             self.cnt-=1
#             self.temp.append(endWord)
#             self.temp.pop()
#             return
#         for i in range(len(wordList)):
#             if not visited[i] and self.diffOneCh(beginWord,wordList[i]):
#                 visited[i]=True
#                 self.cnt+=1
#                 self.temp.append(wordList[i])
#                 self.canSplit(wordList[i],endWord,wordList,visited)
#                 self.temp.pop()
#                 self.cnt-=1
#                 visited[i]=False
#
...
#             visited[wordList.index(beginWord)]=True
#         self.cnt=1
#         self.res=[]
#         self.temp=[]
#         for i,item in enumerate(wordList):
#             if self.diffOneCh(item,beginWord) and not visited[i]:
#                 visited[i]=True
#                 self.cnt+=1
#                 self.temp.append(wordList[i])
#                 self.canSplit(wordList[i], endWord, wordList, visited)
...
#                 visited[i]=False
#                 self.temp.pop()
#         return min(self.res) if len(self.res)!=0 else 0
# s=Solution()



# class Solution:
#     def isOneDiff(self,s,t):
#         cnt=0
...
#                     if self.isOneDiff(v,word):
#                         tmp.add(word)
#             wordList=wordList.difference(tmp)
#             visited=tmp
#             dist+=1
#             if len(tmp)==0:
#                 return 0
